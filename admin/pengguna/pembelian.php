...
<?php 
$pembelian = array();
$ambil = $koneksi->query("SELECT * FROM detail_order INNER JOIN tb_order 
ON detail_order.id_order = tb_order.id_order INNER JOIN produk ON detail_order.id_produk = produk.id_produk INNER JOIN kategori ON detail_order.id_kategori = kategori.id_kategori");
    ...
    <div class="card shadow bg-white">
        <div class="card-body">
            <table class="table table-bordered table-hover tabel-striped">
                ...
                <tr>
                    <td width="90"><?php echo $key+1 ?></td>
                    <td><?php echo $value['username']; ?></td>
                    <td><?php echo $value['nama_produk']; ?></td> 
                    <td><?php echo $value['nama_kategori']; ?></td> 
                    <td><?php echo $value['kuantitas']; ?></td>
                    <td>RP<?php echo number_format($value['hrg_satuan']) ?></td>
                    <td>RP<?php echo number_format($value['total_harga']) ?></td>
                    <td><?php echo date("d F Y", strtotime($value['tgl_pesan'])); ?></td>
                    <td><?php echo $value['status_order']; ?></td>
                    <td><?php echo $value['metode_pembayaran']; ?></td>
                    <td><?php echo $value['metode_pengiriman']; ?></td>
                    <td><?php echo $value['catatan'] ?></td>
                    <td class="text-center" width="220">Alamat</td>
                </tr>
                <?php endforeach ?>
                </tbody>
            </table>
